 ...
 * @Author chenyuxiang
 * @Date 2026-04-16
 * @Description 给你一个 环形 数组 nums 和一个数组 queries 。
 * 对于每个查询 i ，你需要找到以下内容：
...
public class Solution {
    public List<Integer> solveQueries(int[] nums, int[] queries) {
        ...
        for (int query : queries) {
            results.add(getMinDistance(nums, query));
        }
        return results;
    }

    private int getMinDistance(int[] nums, int index) {
        int target = nums[index];
        int n = nums.length;

        // 向左和向右同时搜索,找到任意一个就返回
        for (int i = 1; i < n; i++) {
            int leftIndex = (index - i + n) % n;
            int rightIndex = (index + i) % n;
